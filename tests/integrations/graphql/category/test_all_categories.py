...
def test_all_categories_unauthorized(client_query):
...
@pytest.mark.django_db
def test_all_categories_permission_denied_when_user_is_not_staff(client_query, client):
...
@pytest.mark.django_db
def test_all_categories_permission_denied_when_staff_lacks_view_category_permission(
    client_query, client
):
    user = User.objects.create_user(
...
@pytest.mark.django_db
def test_all_categories_success(client_query, client, setup_system_roles):
    ...
    assert result["data"]["allCategories"]["edges"] == []
