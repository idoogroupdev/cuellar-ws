...
def test_create_staff_user_unauthorized(client_query):
...
@pytest.mark.django_db
def test_create_staff_user_permission_denied_when_user_is_not_staff(
    client_query, client
):
...
@pytest.mark.django_db
def test_create_staff_user_permission_denied_when_staff_lacks_add_user_permission(
    client_query, client
):
    user = User.objects.create_user(
...
@pytest.mark.django_db
def test_create_staff_user_success(client_query, client, setup_system_roles):
    ...
    created_user = User.objects.get(email="staff@example.com")

    assert result["data"]["createStaffUser"]["user"] == {
        "email": "staff@example.com",
        "firstName": "Staff",
